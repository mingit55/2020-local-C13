...
class MainController{
    ...
    // 온라인 집들이
    function partyPage(){
        $sql = "SELECT DISTINCT K.*, user_name, user_id, IFNULL(score, 0) score, M.kid reviewed
                FROM knowhows K
    ...
    function reviewKnowhow(){
        ...
        $data = DB::fetch("SELECT floor(AVG(score)) score, kid FROM knowhow_reviews GROUP BY kid WHERE kid = ?", [$kid]);
        json_response(["score" => $data->score]);
    }

    // 시공 견적
    function estimatePage(){
        $reqList = DB::fetchAll("SELECT Q.*, IFNULL(cnt, 0) cnt, user_name, user_id, M.qid sended
                                FROM requests Q
                                LEFT JOIN users U ON U.id = Q.uid
                                LEFT JOIN (SELECT COUNT(*) cnt, qid FROM responses GROUP BY qid) S ON S.qid = Q.id
                                LEFT JOIN (SELECT kid FROM responses WHERE uid = ?) M ON M.qid = Q.id", [user()->id]);

        $resList = DB::fetchAll("SELECT S.*, user_name, user_id, contents, start_date, sid
                                FROM responses S
